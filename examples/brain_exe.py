...
def example_basic_scan():
    ...
    projections, _ = quick_tomography(
        ...
        "brain_lookup.json", 
        n_projections=360,
        output_file="basic_tomo_brain.h5"
    )
...
def example_full_scan_with_dose():
    ...
    scanner.load_volume("brain_map.zarr", "brain_lookup.json")
...
def example_compare_geometries():
    ...
    # Tomography with golden angles path

def golden_scan(num_proj=100, theta_start=0):
    ...
    # Tomography with golden angles path & interlaced offsets
...
def main():
    ...
    try:
        ...
        import os
        if not os.path.exists("phantom_brain_dose_test.zarr"):
            print("Generating brain phantom...")
            from msim.generate_phantom import generate_phantom
            generate_phantom("brain", shape=(64, 96, 96), voxel_size=(0.5, 0.5, 0.5))
        ...
        print("- basic_tomo_brain.h5 (basic tomography)")
        print("- tomo_with_dose_brain.h5 (tomography + dose)")
